...
### R-2026-05-14-004 - 下載任務分頁與完成通知 UX

- 狀態：完成
- 目標：避免使用者誤關仍在執行的下載任務分頁，並降低批量下載完成時的彈窗干擾。
- 背景：任務分頁原本可直接關閉，即使 worker 還在背景執行；每個任務完成也都會彈出資訊框，批量使用時會打斷操作。
- 目前結論：
  - 總覽分頁不可關閉；非總覽分頁若 worker 執行中，必須先詢問是否停止並關閉。
  - 使用者確認停止並關閉分頁後，該任務後續完成信號不應再彈出完成對話框。
  - 純成功任務用狀態列與總覽 log 即可；只有含失敗的任務才使用 warning 對話框。
- 驗證方式：
  - 以 pytest 覆蓋關閉提示判斷、完成通知層級與摘要文字 helper。
  - 以 PySide offscreen smoke 模擬關閉執行中任務分頁的取消/確認路徑，以及純成功/失敗完成通知行為。
- 驗證結果：
