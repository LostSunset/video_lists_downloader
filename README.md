...
- 🔕 降低下載完成彈窗干擾：純成功任務只更新狀態列與總覽，有失敗時才彈出警告
- ✅ 新增下載任務 UX helper 測試與 PySide offscreen smoke 驗證

### v0.10.0 (2026-05-14)
- 🆕 新增「已記住播放清單」管理 UI，可載入、檢查、開啟資料夾與移除追蹤記錄
- 🆕 「檢查所有播放清單」新增進度列、目前檢查項目與取消檢查按鈕
- 🔧 批次檢查改用可取消的背景 worker，取消後會將尚未處理的清單納入摘要
- ✅ 新增已記住清單管理與批次取消的回歸測試
...
- 🐛 修正 cookie 不可用時播放清單偵測全部失敗的問題（自動 fallback 無 cookie 抓取）
- 🐛 修正批次檢查未比較儲存路徑與 UI 路徑的問題
- ♻️ 提取 `_migrate_playlist_path` 共用方法，統一路徑遷移邏輯

### v0.9.0 (2026-03-24)
- 🆕 播放清單路徑變更偵測：當同一播放清單的下載位置與先前不同時，自動彈出對話框詢問要下載到原路徑、新路徑或瀏覽其他位置
